...
ALE-12 adds the first card definition: **Attack**, costing **1 mana** and dealing **1 damage**, in `rogue-deck/cards/attack.tres`. Its reusable view is `rogue-deck/scenes/attack_card.tscn`. Open `rogue-deck/scenes/card_preview.tscn` and press F6 in Godot to inspect the design. The sword SVG is original project artwork. The card records base statistics and displays them; combat, mana spending, deck composition, and menu navigation to cards remain future work. See [ALE-12 specification](docs/tasks/ALE-12.md) for validation evidence.
